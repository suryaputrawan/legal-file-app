        ...
        <div class="col-12">
            ...
            <div class="card">
                <form action="{{ route('agreement.update',$agreement->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <h4>Edit Data Agreement</h4>
                    </div>
                    <div class="card-body">
                        <div class="form-group">
                            <label>Nama Perjanjian</label>
                            <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ $agreement->name }}" autocomplete="off">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="nomor" id="nomor" class="form-control @error('nomor') is-invalid @enderror" value="{{ $agreement->nomor }}" autocomplete="off">
                            @error('nomor')
                                <div class="mt-2 text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select name="counter_id" id="counter_id" class="form-control @error('counter_id') is-invalid @enderror">
                                ...
                                @foreach ($counter as $item)
                                    <option value="{{ $item->id }}" {{ old('counter_id', $agreement->counter_id) == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('counter_id')
                                <div class="mt-2 text-danger">{{ $message }}</div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="tglterbit" id="tglterbit" class="form-control @error('tglterbit') is-invalid @enderror" value="{{ $agreement->tglterbit }}" autocomplete="off">
                            @error('tglterbit')
                                <div class="mt-2 text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" name="tglexp" id="tglexp" class="form-control @error('tglexp') is-invalid @enderror" value="{{ $agreement->tglexp }}" autocomplete="off">
                            @error('tglexp')
                                <div class="mt-2 text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select name="unit_id" id="unit_id" class="form-control @error('unit_id') is-invalid @enderror">
                                ...
                                @foreach ($unit as $item)
                                    <option value="{{ $item->id }}" {{ old('unit_id', $agreement->unit_id) == $item->id ? 'selected' : null }}>{{ $item->name }} - {{ $item->alias }}</option>
                                @endforeach
                            </select>
                            @error('unit_id')
                                <div class="mt-2 text-danger">{{ $message }}</div>
                        ...
                        <div class="form-group">
                            ...
                            <div id="image-preview" class="image-preview mb-2" style="width: 200px; height:200px">
                                @if ($agreement->picture != null)
                                    <img src="{{ $agreement->takeImage }}" id="image" width="200" alt="Preview Gambar" />
                                @else
