...
<section class="section">
    ...
    <div class="section-body">
        <div class="col-12">
            ...
            <!-- End Flass Message -->
            <div class="card">
              <form action="{{ route('izinCorporate.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                <div class="card-header">
                    <div class="section-header-back">
                        <a href="{{ route('izinCorporate') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
                    </div>
                  <h4>Tambah Data Izin Corporate</h4>
                </div>
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="penerbit_id" id="penerbit_id" class="form-control @error('penerbit_id') is-invalid @enderror">
                            ...
                            @foreach ($penerbit as $item)
                                <option value="{{ $item->id }}" {{ old('penerbit_id') == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('penerbit_id')
                            <div class="mt-2 text-danger">{{ $message }}</div>
