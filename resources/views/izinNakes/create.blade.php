...
<section class="section">
    ...
    <div class="section-body">
        <div class="col-12">
            ...
            <!-- End Flass Message -->
            <div class="card">
              <form action="{{ route('izinNakes.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                <div class="card-header">
                    <div class="section-header-back">
                        <a href="{{ route('izinNakes') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
                    </div>
                  <h4>Tambah Data Izin Nakes</h4>
                </div>
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <select name="employee_id" id="employee_id" class="form-control @error('employee_id') is-invalid @enderror">
                        ...
                        @foreach ($employee as $item)
                            <option value="{{ $item->id }}" {{ old('employee_id') == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('employee_id')
                        <div class="mt-2 text-danger">{{ $message }}</div>
                  ...
                  <div class="form-group">
                    ...
                    <select name="department_id" id="department_id" class="form-control @error('department_id') is-invalid @enderror">
                        ...
                        @foreach ($department as $item)
                            <option value="{{ $item->id }}" {{ old('department_id') == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('department_id')
                        <div class="mt-2 text-danger">{{ $message }}</div>
